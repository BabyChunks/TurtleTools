...
--[2] = length, height and  width of mining quarry -> table

local filePath = "/ChunksWare/libs/"

print("Loading environment...")
Lt = require(filePath.."luatools")
St = textutils.unserialize(fs.open("Chunksware/settings.txt", "r").readAll())
GUI = require(filePath.."GUI")
Comms = require(filePath.."comms")
GPS = require(filePath.."GPS")
Recall = vector.new(table.unpack(textutils.unserialize(arg[1])))

local origin = Lt.tableShallowCopy(Coords)
local delta = vector.new(table.unpack(textutils.unserialize(arg[2])))
